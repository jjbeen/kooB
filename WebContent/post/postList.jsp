...
<body>
	...
	<div class="ui container" id="list">
		...
		<br><br>
		<div class="ui link four stackable fluid cards">
			<c:forEach var="post" items="${postList}">
				<div class="ui card" onclick="location.href='PostServlet?command=post_view&num=${post.num}'">
					<div class="content">
						<div class="header"><i class="window minimize outline icon"></i>${post.title}</div>
						<div class="meta">
							<span class="right floated time"><fmt:formatDate value="${post.writedate}"/></span> <span
								class="category">${post.readcount}</span>
						</div>
						<div class="right floated author">
							<i class="thumbtack icon"></i> ${post.id}
						</div>
					</div>
				</div>
			</c:forEach>
		...
		<button class="ui right floated button" onclick="return checkLogin()">
			<i class="pencil alternate icon"></i>글쓰기
		</button>
	</div>
...
<script>
	...
	$('input[name=query]').keydown(function(key) {
	 ...
	 function checkLogin() {
	        var id = '${loginUser}'; // 수정 ''처리
	        // 수정 ''공백 비교
	        if (id == '') {
	            alert("로그인 후 글쓰기가 가능합니다.");
	            return false;
	        } else {
	            location.href = 'PostServlet?command=post_write_form';
	        }
	    }
